...
class _BaseSoftGradientBoosting(BaseModule):
    def __init__(
        ...
        self.device = torch.device(args['device'] if cuda else "cpu")
        self.n_jobs = n_jobs
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, elem in enumerate(train_loader):
                ...
                output = [estimator(*data)[0] for estimator in self.estimators_]
...
class SoftGradientBoostingClassifier(
    ...
    def forward(self, *x):
        output = [estimator(*x)[0] for estimator in self.estimators_]
...
class SoftGradientBoostingRegressor(_BaseSoftGradientBoosting, BaseRegressor):
    ...
    def forward(self, *x):
        outputs = [estimator(*x)[0] for estimator in self.estimators_]
