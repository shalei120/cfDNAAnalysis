...
import sys
# sys.path.insert(0,'/home/shalei/qingmed/')
...
class BaseModule(nn.Module):
    ...
    def __init__(
        ...
        if len(cuda) ==  1:
            if cuda[0] == 'cpu':
                ...
                self.device = torch.device('cuda:'+str(cuda[0]))
        else:
            self.device = [torch.device('cuda:'+str(d)) for d in cuda]
            self.GPUload = torch.Tensor([0 for _ in self.device])
    ...
    def findfreeGPU(self):
        if  isinstance(self.device, list):
            index = torch.argmin(self.GPUload)
            self.GPUload[index] += 1
            print("Arrange this to GPU:", index)
            return torch.device('cuda:'+str(index.item()))
        else:
            return self.device
    ...
    def _make_estimator(self):
        ...
        # Call `__init__` to make a base estimator
            ...
            # With params
                ...
                self.estimator_args['device'] = GPU_dev
                estimator = self.base_estimator_(**self.estimator_args)

        return estimator.to(GPU_dev)
