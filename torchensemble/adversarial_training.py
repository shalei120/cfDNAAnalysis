...
def _parallel_fit_per_epoch(
    ...
    for batch_idx, elem in enumerate(train_loader):
        ...
        _output, regu = estimator(*data)
        _loss = criterion(_output, target) + regu #+ F.cross_entropy(cali_logit,target)
        _loss.backward()
        ...
        optimizer.zero_grad()
        org_output, regu_ori  = estimator(*data)
        adv_output, regu_adv = estimator(*adv_data)
        loss = criterion(org_output, target) + regu_ori \
               + criterion(adv_output, target) + regu_adv# + F.cross_entropy(cali_logit_adv,target)
        loss.backward()
        optimizer.step()
...
class AdversarialTrainingClassifier(_BaseAdversarialTraining, BaseClassifier):
    ...
    def forward(self, *x):
        ...
        outputs = [
            F.softmax(estimator(*x)[0], dim=1) for estimator in self.estimators_
        ]
    ...
    def fit(
        ...
        def _forward(estimators, *x):
            outputs = [
                F.softmax(estimator(*x)[0], dim=1) for estimator in estimators
            ]
            proba = op.average(outputs)
